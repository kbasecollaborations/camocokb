...
class camocokb:
    ...
    GIT_COMMIT_HASH = "63118809cb3f99ea2b1ca2c96a7b101557cf67d0"
    ...
    def buildgwasobj(self, ctx, params):
        """
        ...
        #BEGIN buildgwasobj
        if 'filename' not in params:
            raise ValueError('No cob file path is given to create GWAS object from.')
        else:
            if not os.path.exists(params['filename']):
                raise FileNotFoundError(f"Input ontology file {params['filename']} does not exist.")
        if 'gwas_name' not in params:
            raise ValueError('GWAS is not define for gwas object generation')
        if 'description' not in params:
            params['description'] = 'GWAS input'
        if 'refgen_name' not in params:
        ...
        gwas_out, err = gwas_proc.communicate()
        ...
        # At some point might do deeper type checking...
        if not isinstance(output, dict):
            raise ValueError('Method buildgwasobj return value ' +
                             'output is not type dict as required.')
        # return the results
        return [output]
